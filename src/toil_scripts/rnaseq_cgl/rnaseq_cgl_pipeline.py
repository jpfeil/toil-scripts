...
def kallisto(job, job_vars):
    ...
    tarball_files(tar_name='kallisto.tar.gz', fpaths=output_files, work_dir=work_dir)
...
def star(job, job_vars):
    ...
    if input_args['wiggle'] and input_args['s3_dir']:
        ...
        tarball_files('wiggle.tar.gz', fpaths=wiggles, work_dir=work_dir)
...
def rsem_postprocess(job, job_vars):
    ...
    tarball_files(tar_name='rsem.tar.gz', fpaths=[os.path.join(work_dir, x) for x in output_files], work_dir=work_dir)
