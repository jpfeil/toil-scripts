...
def parse_input_samples(job, inputs):
    ...
    inputs: Namespace       Stores input arguments (see main)
    """
    samples = []
    if inputs.config:
    ...
    job.addChildJobFn(job_batcher, inputs, samples)


def job_batcher(job, inputs, samples):
    """
    Spawns a tree of jobs to avoid overloading the number of jobs spawned by a single parent.


    inputs: Namespace       Stores input arguments
    samples: list           list of tuples that contian sample information
    """
    num_partitions = 100
    partition_size = len(samples) / num_partitions
    if partition_size > 1:
        for partition in partitions(samples, partition_size):
            job.addChildJobFn(job_batcher, inputs, partition)
    else:
        for sample in samples:
            job.addChildJobFn(download_sample, inputs, sample)


def download_sample(job, inputs, sample):
    """
    Downloads input sample

    inputs: Namespace       Stores input arguments
    sample: tuple           (uuid, URL) of a sample
    """
    uuid, url = sample
    # Download sample
    if urlparse(url[0]) == 'file':
        tar_id = job.fileStore.writeGlobalFile(urlparse(url[0]).path)
    elif inputs.ssec:
        tar_id = job.addChildJobFn(download_encrypted_file_job, url, inputs.ssec, disk='30G').rv()
    else:
        tar_id = job.addChildJobFn(download_from_url_job, url, disk='30G').rv()
...
def process_sample(job, inputs, tar_id):
    ...
    inputs: Namespace       Stores input arguments
    tar_id: str             FileStore ID of sample tar
    """
    # Unpack variables
...
def cutadapt(job, inputs, r1_id, r2_id):
    ...
    inputs: Namespace       Stores input arguments
    r1_id: str              Read 1 fastq
    r2_id: str              Read 2 fastq
    """
    work_dir = job.fileStore.getLocalTempDir()
    ...
    # start STAR
    job.addChildJobFn(star, inputs, r1_cutadapt, r2_cutadapt, cores=12, disk='100G', memory='40G').rv()
...
def star(job, inputs, r1_cutadapt, r2_cutadapt):
    ...
    inputs: Namespace       Stores input arguments
    r1_cutadapt: str        Read 1 fastq (processed)
    r2_cutadapt: str        Read 2 fastq (processed)
    """
    work_dir = job.fileStore.getLocalTempDir()
    # Retrieve files
    ...
    # Get starIndex
    download_from_url(inputs.star_index, work_dir, 'starIndex.tar.gz')
...
def variant_calling_and_qc(job, inputs, bam_id, bai_id):
    """
    Performs variant calling with samtools and QC with CheckBias.

    inputs: Namespace       Stores input arguments
    bam_id: str             FileStore ID of bam
    bai_id: str             FileStore ID of index file
    """
    work_dir = job.fileStore.getLocalTempDir()
    ...
    for url, fname in input_info:
        download_from_url(url, work_dir=work_dir, name=fname)
    ...
    tarball_files('vcqc.tar.gz', work_dir=work_dir, fpaths=[output_tsv, output_vcf])
    return job.fileStore.writeGlobalFile(os.path.join(work_dir, 'vcqc.tar.gz'))


def spladder(job, inputs, bam_id, bai_id):
    """
    Runs Spladder to detect and quantify alternative splicing events.

    inputs: Namespace       Stores input arguments
    bam_id: str             FileSotre ID of bam
    bai_id: str             FileStore ID of index file
    """
    work_dir = job.fileStore.getLocalTempDir()
    # Pull in alignment.bam from fileStore
    ...
    job.fileStore.readGlobalFile(bai_id, os.path.join(work_dir, 'alignment.bam.bai'))
    # Download input file
    download_from_url(url=inputs.gtf, work_dir=work_dir, name='annotation.gtf')
    download_from_url(url=inputs.gtf_pickle, work_dir=work_dir, name='annotation.gtf.pickle')
    # Call Spladder
    command = ['--insert_ir=y',
               '--insert_es=y',
               '--insert_ni=y',
    ...
    tarball_files('spladder.tar.gz', work_dir=work_dir, fpaths=[output_pickle, output_filt, output])
...
def consolidate_output_tarballs(job, inputs, vcqc_id, spladder_id):
    ...
    inputs: Namespace       Input argumnets
    vcqc_id: str            FileStore ID of variant calling and QC tarball
    spladder_id: str        FileStore ID of spladder tarball
    """
    work_dir = job.fileStore.getLocalTempDir()
    # Retrieve IDs
    uuid = inputs.uuid
...
def main():
    ...
    # Store inputs from argparse in Namespace object
